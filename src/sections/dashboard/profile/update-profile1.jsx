...
export default function UpdateProfileDialog({ open, onClose }) {
  ...
  const [avatarFile, setAvatarFile] = useState(null); // Blob or File
  ...
  const [cropImageSrc, setCropImageSrc] = useState(null); // what Cropper displays
  const [crop, setCrop] = useState({ x: 0, y: 0 });
  const [zoom, setZoom] = useState(1);
  const [croppedAreaPixels, setCroppedAreaPixels] = useState(null);

  // sync with latest user whenever dialog opens or user changes
  useEffect(() => {
    if (!open) return;
  ...
  const handleEditAvatarClick = () => {
    ...
    setCropImageSrc(previewUrl); // use current avatar URL
    setCrop({ x: 0, y: 0 });
    setZoom(1);
    setCroppedAreaPixels(null);
  ...
  // 📤 Change Avatar → upload NEW image then crop
